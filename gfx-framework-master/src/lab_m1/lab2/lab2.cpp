...
void Lab2::Init()
{
    ...
    // by definition!
    {
        ...
        vector<glm::vec3> points{
            glm::vec3{0,1,0},
            glm::vec3{0.942,-.3333,0},
            glm::vec3{-0.471,-.3333,-0.816},
            glm::vec3{-0.471,-.3333,0.816}
        };

        vector<VertexFormat> vertices
        {
            VertexFormat(points[0], glm::vec3(1, 0, 0)),
            VertexFormat(points[1], glm::vec3(1, 0, 0)),
            VertexFormat(points[2], glm::vec3(1, 0, 0)),

            VertexFormat(points[0], glm::vec3(0, 1, 0)),
            VertexFormat(points[2], glm::vec3(0, 1, 0)),
            VertexFormat(points[3], glm::vec3(0, 1, 0)),

            VertexFormat(points[0], glm::vec3(0, 0, 1)),
            VertexFormat(points[3], glm::vec3(0, 0, 1)),
            VertexFormat(points[1], glm::vec3(0, 0, 1)),

            VertexFormat(points[3], glm::vec3(.5f, .5f, .5f)),
            VertexFormat(points[2], glm::vec3(.5f, .5f, .5f)),
            VertexFormat(points[1], glm::vec3(.5f, .5f, .5f)),
        };
...

    {
        ...
        vector<VertexFormat> vertices
        {
            VertexFormat({-1,0,-1}, glm::vec3(1, 1, 1)),
            VertexFormat({-1,0,1}, glm::vec3(1, 1, 1)),
            VertexFormat({1,0,1}, glm::vec3(1, 1, 1)),
            VertexFormat({1,0,-1}, glm::vec3(1, 1, 1)),
        ...
        // Actually create the mesh from the data
        CreateMesh("squar", vertices, indices);
    }
...
void  Lab2::animateTetra() {
#define animated(offs) (sin(timeT*2 + AI_MATH_TWO_PI *offs) / 2 + .5)

    {
        ...
        vector<VertexFormat> vertices
        {
            VertexFormat(points[0], glm::vec3(animated(1), animated(1), animated(1))),
            VertexFormat(points[1], glm::vec3(animated(1 / 4), animated(1), animated(1))),
            VertexFormat(points[2], glm::vec3(animated(2 / 4), animated(1), animated(1))),

            VertexFormat(points[0], glm::vec3(animated(1), animated(1),  animated(1))),
            VertexFormat(points[2], glm::vec3(animated(1), animated(2 / 4),  animated(1))),
            VertexFormat(points[3], glm::vec3(animated(1), animated(3 / 4),  animated(1))),

            VertexFormat(points[0], glm::vec3(animated(1),  animated(1), animated(1))),
            VertexFormat(points[3], glm::vec3(animated(1),  animated(1), animated(3 / 4))),
            VertexFormat(points[1], glm::vec3(animated(1),  animated(1), animated(1 / 4))),

            VertexFormat(points[3], glm::vec3(animated(3 / 4))),
            VertexFormat(points[2], glm::vec3(animated(2 / 4))),
            VertexFormat(points[1], glm::vec3(animated(1 / 4))),
        };
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_A"], shaders["VertexColor"], glm::vec3(-1.5f, 0.5f, 0), glm::vec3(0.5f));
    ...
    auto rot = glm::rotate(glm::identity<glm::mat4>(), timeT, { 0,1,0 });
    rot = glm::translate(rot, { 0,2,0 });


    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tetra"], shaders["VertexColor"], rot);
    // TODO(student): Draw the square
    RenderMesh(meshes["squar"], shaders["VertexColor"], glm::vec3(0, 0, 0), glm::vec3(1));
