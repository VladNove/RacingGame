...
void main()
{
    ...
    vec3 pv = (Model * vec4(v_position,1.0)).xyz; // world position of particle
    ...
    // only draw particle if this is true
    bool keep = false;

    // blend the color to bg if particle is old
    vec3 pcolor = v_color + (bgColor - v_color) * min(tphyis * 10,0.6);
