...
void Lab5::Init()
{
...

    {
        ...
        mesh->LoadMesh(PATH_JOIN(window->props.selfDir, RESOURCE_PATH::MODELS, "primitives"), "untitled.obj");
        meshes[mesh->GetMeshID()] = mesh;
    }
    ...
    if (!ortho)
        ...
        projectionMatrix = glm::ortho(0, 0, window->GetResolution().x, window->GetResolution().y, 1, 200);
}
...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["car"],shaders["VertexColor"], glm::translate(glm::mat4(1), glm::vec3(0, 0, 0)));
...

    {
        ...
        modelMatrix = glm::scale(modelMatrix, { 1.5,1.5,1.5 });
...
void Lab5::OnKeyPress(int key, int mods)
{
    ...
    if (!ortho)
        ...
        projectionMatrix = glm::ortho(-orthoX, orthoX, -orthoY , orthoY,0.f,200.f);
}
