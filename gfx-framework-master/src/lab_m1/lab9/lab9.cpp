...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        glm::vec3 toCamera = cameraPos - grassPos;
        toCamera.y = 0;
        toCamera = glm::normalize(toCamera);
        modelMatrix *= glm::toMat4(glm::rotation(glm::vec3{ 0,0,1 }, toCamera));
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2, bool earth)
{
    ...
    if (texture2)
    {
        ...
        glUniform1i(glGetUniformLocation(shader->program, "secondTexture"), 1);

    }
    else
    {
        glUniform1i(glGetUniformLocation(shader->program, "secondTexture"), 0);
    }

    if (earth) {
        glUniform1i(glGetUniformLocation(shader->program, "earth"), 1);
        glUniform1f(glGetUniformLocation(shader->program, "time"), Engine::GetElapsedTime());


    }
    else {
        glUniform1i(glGetUniformLocation(shader->program, "earth"), 0);
