...
{  
    ...
    vec2 texc0 = texcoord * 10  + textureOffs / 10;
    vec2 texc = texcoord * 100  + textureOffs;
    ...
    out_color = vec4(vec3(texture2D(texture, texc)*.5 + texture2D(texture, texc0)*.5) * light,1);
